"""
diary-survey-bot | survey-editor

Software-Design: Philipp Feldner
Documentation: https://github.com/Catrobat/diary-survey-bot

Qt version: 5.12.1
"""


block_settings = ["MANDATORY"]

question_commands = [("Force Keyboard Reply", ["FORCE_KB_REPLY"]), ("Continue automatic queue", ["Q_ON"]),
                     ("Store country information", ["COUNTRY"]), ("Store gender information", ["GENDER"]),
                     ("Store timezone information", ["TIMEZONE"]), ("Store age information", ["AGE"])]

scheduling = ["05:00", "05:30", "06:00", "06:30",
              "07:00", "07:30", "08:00", "08:30",
              "09:00", "09:30", "10:00", "10:30",
              "11:00", "11:30", "12:00", "12:30",
              "13:00", "13:30", "14:00", "14:30",
              "15:00", "15:30", "16:00", "16:30",
              "17:00", "17:30", "18:00", "18:30",
              "19:00", "19:30", "20:00", "20:30",
              "21:00", "21:30", "22:00", "22:30",
              "random (08-10)", "random (11-13)",
              "random (15-18)", "random (19-22)"]
